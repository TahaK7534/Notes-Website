...
function WriteBox({ edit }) {
    ...
    noteId -= 1;
    const [notes, updateNote, deleteNote, profile, user] = useOutletContext();
    ...
    const save = async () => {
        ...
        const res = await fetch(
            `https://xk2sn6nqldt6hbxk5zy3npl5km0tywvi.lambda-url.ca-central-1.on.aws?email=${profile}&auth_token=${user.access_token}`,
            {
                method: "POST",
                headers: {
                    "Content-Type": "application/json"
                },
                body: JSON.stringify({ ...new_note, email: profile })
            }
        );
    };
